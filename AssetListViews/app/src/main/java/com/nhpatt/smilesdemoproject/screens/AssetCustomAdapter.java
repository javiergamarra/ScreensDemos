...
public class AssetCustomAdapter extends AssetListAdapter {
	...
	private Picasso picasso = new Picasso.Builder(LiferayScreensContext.getContext()).downloader(
		new OkHttpDownloader(UnsafeOkHttpClient.getUnsafeClient())).build();
	...
	@Override
	protected void fillHolder(AssetEntry assetEntry, ViewHolder holder) {

		WebContent webContent = (WebContent) assetEntry;
		...
		picasso.load(LiferayServerContext.getServer() + localized).into(smilesHolder._image);
